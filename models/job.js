...
class Job {
     ...
     * 
     * Optional filter using queryFilter.
     * 
     */
    ...
    static async findAll(queryFilters = {}) {
        ...
        const { minSalary, hasEquity, title } = queryFilters;

        if (minSalary !== undefined) {
            queryValues.push(minSalary);
            whereQueries.push(`salary >= $${queryValues.length}`);
        }

        if (hasEquity === true) {
            whereQueries.push(`equity > 0`);
        }

        if (title !== undefined) {
            queryValues.push(`%${title}%`);
            whereQueries.push(`title ILIKE $${queryValues.length}`);
        }
